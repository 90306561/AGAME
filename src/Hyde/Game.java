...
public class Game extends JPanel implements KeyListener {
	...
	public Game() {
		...
		buildings.add(new Building("Gun Shop", Color.yellow, 420, 420, 100, 100));
				...
				if (keysPressed['Q']) {
					...
					for (int i = 0; i < people.size(); i++) {
						if (people.get(i).isAlive()) {
							if (people.get(i).getId() == 4) {
								para++;
							} else if (people.get(i).getId() == 2) {
								police++;
							} else { civilians++;}
						}
					...
					text="Civilians:"+civilians+" Police:"+police+" Paramedics:"+para;
				}
...
//System.out.println(y);
				...
				for (Building b : buildings) {
					if (p.getId() != 2&&b.touches(p.getC()) || p.getC().touches(b) || p.getX() < 50 + 240 - x
						...
						if (
								b.getName().equals("Bar")&&Math.random()<0.1&&p.getId()!=4) {
							p.setDrunk(true);
						}
				...
				if (player.getC().touches(p.getC())) {
					...
					if (!isHyde && keysPressed[32]) {
						...
						if(p.isWanted()) {
							...
							p.setY(2400);
							p.setWanted(false);
							money+=(int)(20*Math.random());
							System.out.println("YOU ARRESTED A MURDERER!");
						}
						
					} if (startDigging == 0 && isHyde && keysPressed['G']) {
						startDigging=System.currentTimeMillis();
				
						
					}
					
					if (startDigging!=0&&player.getC().touches(p.getC())) {
						if(System.currentTimeMillis()-startDigging>=5000) {
							if (!p.isAlive()) {
								p.setGrave(true);
								startDigging = 0;
							}
...
for (Person t : people) {
	if(hasGun) {
		...
		if(keysPressed[39]) {
			...
			} if (building.equals("Gun Shop")) {
				g.setColor(Color.yellow);
				g.fillRect(0, 0, 500, 500);
				g.setColor(Color.black);
				g.drawString("Pay 200 for a gun (Press K)", 50, 50);
				if (keysPressed[75]) {
					if (money >= 200) {
						money -= 200;
						hasGun = true;
					...
					} else g.drawString("You need more money", 100, 100);
				}
			}
